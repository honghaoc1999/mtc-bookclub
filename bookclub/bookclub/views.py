...
# SELF DEFINED
from bookclub.models import Comment, Post, Profile, Room
from bookclub.forms import ProfileForm, RegisterForm, LoginForm, PostForm, RoomForm
...
@login_required
def room_action(request, room_user_name):
    print(room_user_name)
    name_split = room_user_name.find('+')
    if (name_split == -1):
        return render(request, 'error.html', {
            "errorMessage": "Wrong room name or user name entry."
        })
    room_id = room_user_name[:name_split]
    user_name = room_user_name[name_split+1:]

    # Here is to check if user tries to enter non-existent room or enters a room
    # with other usernames
    room_id_check = Room.objects.filter(id=room_id).count() > 0
    user_name_check = user_name == request.user.username

    room = Room.objects.get(id=room_id);
    print("TITLE", room.book_title)
    if (room_id_check and user_name_check):
        return render(request, 'bookclub/chatroom.html', {
            "room_name": room_id,
            "room_info": room,
            "user_name": user_name,
            "book_cover_link": "/book_cover/" + room_id,
        })
...
def get_global(request):
    ...
    for post_item in Post.objects.all():
        book = Room.objects.get(id=post_item.room_id);
        my_item = {
            'text': post_item.text,
...
def get_rooms(request):
    ...
    for room_item in Room.objects.all():
        my_item = {
            'book_title': room_item.book_title,
            'book_author': room_item.book_author,
            'book_description': room_item.book_description,
            'book_cover': room_item.book_description,
            'content_type': room_item.content_type,
            'admin': room_item.admin,
            'id': room_item.id
        }
        response_data['rooms'].append(my_item)
...
def new_post_action(request):
    ...
    print("After validate: cleaned data", form.cleaned_data)

    pdf = form.cleaned_data['pdf']
    # print("uploaded pdf: {} (type={})".format(pdf, type(pdf)))
    new_post = Post(text=request.POST['text'], user=request.user, creation_time=timezone.now(), room_id=request.POST['book_id'])

    if (pdf != None):
        new_post.pdf = pdf
        new_post.content_type = pdf.content_type
...
@login_required
def new_room_action(request):
    if (request.method == "GET"):
        context = {
            'user': request.user,
            'form': RoomForm(),
        }
        return render(request, 'bookclub/createRoom.html', context);
    ...
    if not form.is_valid():
        ...
        return render(request, 'bookclub/discussionboard.html', context);

    print("After validate: cleaned data", form.cleaned_data)

    book_cover = form.cleaned_data['book_cover']
    print("uploaded book_cover: {} (type={})".format(book_cover, type(book_cover)))

    new_room = Room(book_title=request.POST['book_title'], admin=request.user.username, book_author=request.POST['book_author'], book_description=request.POST['book_description'])

    if (book_cover != None):
        new_room.book_cover = book_cover
        new_room.content_type = book_cover.content_type

    new_room.save();

    context = { 'user': request.user };
    return redirect(reverse('discussionboard'));
...
@login_required
def get_book_cover(request, id):
    item = get_object_or_404(Room, id=id)
    # print('Picture #{} fetched from db: {} (type={})'.format(profile.id, item.picture, type(item.picture)))

    if not item.book_cover:
        raise Http404

    return HttpResponse(item.book_cover, content_type=item.content_type)

@login_required
def get_pdf(request, id):
    post = Post.objects.get(id=id);
    print('Pdf #{} fetched from db: {} (type={})'.format(post.id, post.pdf, type(post.pdf)))

    if not post.pdf:
        raise Http404

    return HttpResponse(post.pdf, content_type=post.content_type)
...
@login_required
def discussionboard_action(request):
    context = {
        'user': request.user,
        'form': RoomForm()
    }
