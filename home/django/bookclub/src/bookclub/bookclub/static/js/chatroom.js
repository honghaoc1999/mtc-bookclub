...
var roomUserName = pathname.substring("/chatroom/".length, pathname.length - 1);
var nameSplit = roomUserName.indexOf("+");
var roomname = roomUserName.substring(0, nameSplit);
var username = roomUserName.substring(nameSplit + 1, roomUserName.length);
window.setInterval(checkPeerConnectionStates, 1000);
  ...
  .then((stream) => {
    ...
    btnToggleAudio.addEventListener("click", () => {
      ...
      if (audioTracks[0].enabled) {
        btnToggleAudio.innerHTML = "Turn Audio Off";
        return;
      }
      btnToggleAudio.innerHTML = "Turn Audio On";
    });
...
function checkPeerConnectionStates() {
  /// call your function here
  var peerNames = Object.keys(mapPeers);
  peerNames.forEach(peerName => {
    console.log(mapPeers[peerName])
    var peer = mapPeers[peerName][0];
    ...
    if (iceConnectionState == 'failed' || iceConnectionState == 'disconnected' || iceConnectionState == 'closed') {
      ...
      var inactiveVideo = document.getElementById('id-'+peerName+'-video');
      if (inactiveVideo != undefined) {
        console.log("removed in check");
        removeVideo(inactiveVideo);
      }
    }
  });
}
...
function webSocketOnMessage(event) {
  ...
  if (action == 'room-full') {
    console.log("exiting")
    window.location.pathname = "discussionboard";
  }
  ...
  if (action == 'new-peer') {
    ...
    if (Object.keys(mapPeers).length < 3) {
  ...
  if (action == 'request-info-from-new-peer') {
...
// var labelRoomname = document.getElementById('label-roomname')
// labelRoomname.innerHTML = roomname;
...
var endPoint = wsStart + loc.host + '/ws/' + roomname;
...
function createOfferer(peerUsername, receiver_channel_name) {
  ...
  const configuration = {
    "iceServers": [{
      "url": "turn:162.243.160.46:3478",
        "credential": "Chh@990713",
          "username": "tom"
    }]
  };
  ...
  if (mapPeers[peerUsername] != undefined) {
    console.log("detected dup", mapPeers[peerUsername][0].iceConnectionState);
    var existingPeerConnection = mapPeers[peerUsername][0];
    var iceConnectionState = existingPeerConnection.iceConnectionState;
    delete mapPeers[peerUsername];

    if (iceConnectionState != 'closed') {
      existingPeerConnection.close();
    }
    var inactiveVideo = document.getElementById('id-'+peerUsername+'-video');
    console.log(inactiveVideo)
    if (inactiveVideo != undefined) {
      console.log("removed in answerer");
      removeVideo(inactiveVideo);
    }
  }
  ...
  dc.addEventListener("open", () => {
    console.log("offerer connection opened!");
  });
  ...
  peer.addEventListener("icecandidate", (event) => {
    if (event.candidate) {
      ...
      console.log('New ice candidate in offerer: ');
      return;
    }
    ...
    setTimeout(() => {
      ...
      peer.addEventListener("datachannel", (e) => {
        ...
        peer.dc.addEventListener("open", () => {
          console.log("answerer connection opened!");
        });
      ...
      peer.addEventListener("icecandidate", (event) => {
        if (event.candidate) {
          // when there is a new ice candidate, the localdescrip for the peer
          // connection will change
          console.log('New ice candidate in answerer: ');
          return;
        }
        // when gathering ice candidates is completed, event.candidate will be null,
        // we reach the code below. We send out sdp to remote peer.
...
function addLocalTracks(peer) {
  var emptyTracks = true;
  if (localStream.getTracks().length == 0) {
    setTimeout(() => {addLocalTracks(peer);}, 3000);
  }
  else {
    localStream.getTracks().forEach(track => {
      ...
      try {
        ...
        console.log("Sending localtracks already succeed")
      }
...
// UI function for creating video
function createVideo(peerUsername) {
  for (let i = 0; i < 3; i++) {
    var videoContainer = document.getElementById("id_inactive_video_"+i);
    if (videoContainer != undefined) {
      videoContainer.id = "id-"+ peerUsername + "-video";
      videoContainer.autoplay = true;
      videoContainer.playsInline = true;
      videoContainer.width="300";
      videoContainer.height="150";
      var usernameContainer = document.getElementById("id_username_"+i);
      usernameContainer.innerHTML = peerUsername;
...
function removeVideo(video) {
  ...
  var inactive_video = document.createElement("video");
  var container_id = videoWrapper.id.substr(-1);
  console.log(videoWrapper.id)
  inactive_video.id = "id_inactive_video_"+ container_id;
  inactive_video.autoplay = true;
  inactive_video.playsInline = true;
  inactive_video.width = "300";
  inactive_video.height = "150";
  videoWrapper.prepend(inactive_video);
  var usernameContainer = document.getElementById("id_username_"+container_id);
  console.log("id_username_"+container_id)
  usernameContainer.innerHTML = "Waiting...";
